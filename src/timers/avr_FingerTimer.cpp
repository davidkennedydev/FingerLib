...
void _passMotorPtr(void (*f)(void)) { _ptr2MotorFunc = f; }

// function to receive pointer for timer piggybacking and assign to global
// pointer
void _attachFuncToTimer(void (*f)(void)) { _ptr2PiggybackFunc = f; }
...
ISR(TIMER5_COMPA_vect) {
  static long timer5cnt =
      0; // main timer counter increments every call of the interrupt
  static long motorCount =
      0; // time instance variable for motor position control
...
void _posCtrlTimerSetup(void) {
  ...
  OCR1A = CC_REG_VAL;     // set compare match register to the value set by
                          // avr_FingerTimer.h
