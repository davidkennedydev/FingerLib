 ...
 *
 * The timer and delay unit is designed to indicate elapsed duration, and can be
 * used as a non-blocking delay function or to measure the amount of time a
 * process has taken. The timer and delay classes are compatible with both
 * milliseconds and microseconds by creating instances of any of the following;
 *
 ...
 *
 * The timer and delay classes use a virtual 'ticker()' function that is used
 * for the duration calculation. This ticker() function is set to return a
 * timestamp in millisecond or microseconds, but can also be spoofed by the unit
 * test process.
 *
 */
...
public:
  ...
  bool timeElapsed(long delVal); /**< run the timer constantly, return true
                                    every time the delVal has elapsed */

  long
  getInterval(void); /**< return the interval/delVal currently being used */
...
private:
  ...
  virtual unsigned long ticker(void) /**< virtual function used to track the
                                        change in time (us, ms, seconds etc) */
  {
    return 0;
  }
};
...
public:
  ...
  bool started(void); /**< returns true if delay timer is running */
  long now(void); /**< returns how much time has elapsed since the start, if not
                     started return 0 */
  long stop(void);    /**< stop the timer and return elapsed time */
  long restart(void); /**< stop the timer, return the elapsed time and start the
                         timer again */

  bool timeElapsed(long interval); /**< check whether a certain interval has
                                      elapsed, keep timer running */
...
private:
  ...
  ticker(void) /**< function used to increment the timer after a particular
                  period (us, ms, seconds etc) */
  {
    return 0;
  }
};
