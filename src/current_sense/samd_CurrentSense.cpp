...
void initCurrentSense(uint8_t dir0, uint8_t dir1, void (*f)(void)) {
  ...
  // increment the list to show which timers are being used for PWM, so they can
  // be synced later
  if (timer0 != -1)
    PWMTimerList[timer0]++;
...
// change the ADC clock prescaler to DIV (to reduce time in curr sense
// interrupt)
...
// count through every timer, if it is being used for PWM (timerList[i] > 0)
// disable timer and reset all counters to 0 when every PWM timer is clear,
// re-enable all PWM timers and return number of synced PWM timers
...
//		}
//
//		TC->INTFLAG.bit.OVF = 1;    // writing a one clears the flag ovf
//flag
//	}
//}
