...
void PID_CONTROLLER::enable(void) { enable(true); }

// disable the PID controller
void PID_CONTROLLER::disable(void) { enable(false); }

// set the PID controller to be enabled/disabled
void PID_CONTROLLER::enable(bool en) { _en = en; }
...
double PID_CONTROLLER::run(double targ, double curr) {
  ...
  if (sampleTime > 0) {
    // integral summation allows _Ki to change without causing unwanted
    // disturbances
...
void PID_CONTROLLER::setRampRate(double ramp) { _rampLim = ramp; }
