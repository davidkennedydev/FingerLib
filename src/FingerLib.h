...
// CHANGE SETTINGS
// Uncomment out the following to use PID pos control instead of custom P
// control
#define USE_PID
...
#define VEL_BUFF_SIZE                                                          \
  16 // number of values to store in velocity smoothing buffer (must be a power
     // of 2)
...
#if defined(FORCE_SENSE)
// STALL DETECTION
#define MAX_STALL_TIME_MS                                                      \
  750 // ms. maximum amount of time the motors can stall for before they are
      // determined to be stalled
#define STALL_CURRENT_THRESH                                                   \
  50 // current threshold, value, above which, if the motor velocity == 0, the
     // motor is deemed stalled

// CURRENT
#define MAX_CURR_VAL MAX_ADC_RES
#define CURR_SPIKE_DUR_US (double)50000 // us - duration of peak to discard
#define CURR_SENSE_BUFF_SIZE                                                   \
  64 // number of samples to store in the current sense circle buffer
...
#define ADC_VALS_PER_MA_DRAW                                                   \
  (float)(CURRENT_SENSE_SATURATION_VAL /                                       \
          MOTOR_DRIVER_CURRENT_LIMIT) // number of ADC values per mA draw of
                                      // motor

// Y = MX + C, where Y = force, X = current draw
#define CURRENT_SENSE_CONST_M                                                  \
  (float)7.4833 // generated from the equation of the line from the
                // force-current graph
#define CURRENT_SENSE_CONST_C                                                  \
  (float)46.444 // generated from the equation of the line from the
                // force-current graph
#endif

// MACROS
#define window(amt, low, high)                                                 \
  ((amt) < (low)                                                               \
       ? (0)                                                                   \
       : ((amt) > (high) ? (0) : (amt))) // limit amt to within low and high
...
public: // TODO Review what function members should really be public
  ...
  // INITIALISATION
  uint8_t
  attach(uint8_t dir0, uint8_t dir1,
         uint8_t posSns); // attach pins to a finger using only position control
  uint8_t attach(uint8_t dir0, uint8_t dir1, uint8_t posSns,
                 bool inv); // attach pins to a finger using only position
                            // control, but allow the direction to be inverted
  uint8_t
  attach(uint8_t dir0, uint8_t dir1, uint8_t posSns, uint8_t forceSns,
         bool inv);  // attach pins to a finger using position control and force
                     // control, and allow the direction to be inverted
  void detach(void); // deactivate the finger
  bool attached(void); // return true if the current finger is attached and
                       // initialised correctly
  void invertFingerDir(void); // set the motor to be inverted

  // LIMITS
  void setPosLimits(int min,
                    int max); // set the maximum and minimum position limits
  void setPWMLimits(int min,
                    int max); // set the maximum and minimum speed limits
#ifdef FORCE_SENSE
  void setForceLimits(int min,
                      int max); // set the maximum and minimum force limits
#endif
  ...
  int16_t readPosError(void);   // return the error between the current position
  ...
  int readPWM(
      void); // return the current speed being written to the motor (PWM)
  int readTargetPWM(void); // return the target motor speed (PWM)

#ifdef FORCE_SENSE
  // FORCE
  float readForce(void); // return the current force value. If force sense is
                         // not enabled, return blank (-1)
  uint16_t readCurrent(void); // return the latest force sense ADC value
  bool
  reachedForceLimit(void); // return true if the force limit has been reached
  float convertADCToForce(
      int ADCVal); // convert ADC current sense value to force value (float)
  int convertForceToADC(
      float force); // convert force value to ADC current sense value (int)
#endif
  ...
  //// PRINT
  // void printPos(void);				// print the current
  // position (no new line) void printPos(bool newL);			// print
  // the current position (new line) void printPosError(void);
  // // print the current position error (no new line) void printPosError(bool
  // newL);	// print the current position error (new line)

  // void printDir(void);				// print the current
  // direction (no new line) void printDir(bool newL);			// print
  // the current direction (new line) void printReached(void);
  // // print whether the target position has been reached (no new line) void
  // printReached(bool newL);		// print whether the target position has
  // been reached (new line)

  // void printSpeed(void);			// print the current speed (no
  // new line) void printSpeed(bool newL);		// print the current
  // speed (new line)
  //
  // void printDetails(void);			// print current position,
  // direction, speed and whether the target position has been reached void
  // printConfig(void);			// print finger number, pins and limits

  // CONTROL
  void
  control(void); // run position (and force) controller (called via interrupt)
#ifdef FORCE_SENSE
  void calcCurrentSns(void); // read the current force and discount the current
                             // spike (called via interrupt)
#endif
...
private:
  ...
  VectorProperties
      _speed; // movement speed properties (prev, curr, targ, error, limit)
  VectorProperties
      _PWM; // PWM speed properties (prev, curr, targ, error, limit)
#ifdef FORCE_SENSE
  VectorProperties _force; // force properties (prev, curr, targ, error, limit)
                           // (all stored as ADC values instead of N)
#endif
  ...
  bool _interruptEn; // flag to set whether to use the timer interrupt for motor
                     // control
#ifdef FORCE_SENSE
  ...
  void
  motorControl(int speed); // split the vectorised motor speed into direction
                           // and speed values and write to the motor
#ifdef FORCE_SENSE
  bool
  stallDetection(void); // if the finger is drawing too much current whilst not
                        // moving, set the target pos to the current pos
  void forceController(void); // stop the finger if the force limit is reached,
                              // or move to reach a target force
#endif
...
void _fingerControlCallback(
    void); // runs the control() function of a Finger instance at each call by
           // the timer interrupt
#ifdef FORCE_SENSE
void _currentSenseCallback(void); // runs the calcCurrentSns() function of a
                                  // Finger instance at each call
#endif

// HARDWARE SPECIFIC FUNCTIONS
#ifdef ARDUINO_AVR_MEGA2560
void setPWMFreq(uint8_t pin, uint8_t value); // change the PWM timer frequency
                                             // to be out of the audible range
#endif
