...
// #define C_BUFF_MAX_SIZE		64				//
// default buffer size  (2^6) #define C_BUFF_MAX_SIZE		128
// // default buffer size  (2^7) #define C_BUFF_MAX_SIZE		255
// // default buffer size  (2^8) #define C_BUFF_MAX_SIZE		512
// // default buffer size  (2^9)
#define C_BUFF_MAX_SIZE 1024 // default buffer size  (2^10)
// #define C_BUFF_MAX_SIZE		2048			// default
// buffer size  (2^11) #define C_BUFF_MAX_SIZE		4096
// // default buffer size  (2^12) #define C_BUFF_MAX_SIZE		8192
// // default buffer size  (2^13)

/**
 * # Circle buffer class
 *
 * The circle buffer unit is a buffer in which data can be stored. The buffer is
 * instantiated to a set type and initialised to a set size, where data can then
 * be stored. Each new data entry is stored in the next buffer 'slot', and when
 * the buffer is full, the buffer wraps around and overwrites the first data
 * entry. Thus the buffer will not overflow.
 *
 * The circle buffer also calculates a running total, running mean, minimum and
 * maximum value of all data entries.
 *
 */

template <class buff_type> class CIRCLE_BUFFER {
public:
  CIRCLE_BUFFER() {
  ...
  ~CIRCLE_BUFFER() { destroy(); }

  /**  Create an instance of a circular buffer of size 'size' (size must be a
   * power or 2)
   */
  bool begin(uint16_t size) {
    ...
    if (_buff == NULL) {
      ...
      // initialise all values to 0, so that when it is subtracted from the
      // total on the first write nothing happens
      fill(0);
  ...
  /**  Read the first (oldest) value from the buffer and don't increment read
   * index
   */
   ...
   */
  buff_type readMean(void) { return _mean; }

  /**  Read the total of the buffer
   */
  long double readTotal(void) { return _total; }

  /**  Read the minimum value of the buffer
   */
  buff_type readMin(void) { return _min; }

  /**  Read the maximum value of the buffer
   */
  buff_type readMax(void) { return _max; }

  /**  Write to the buffer, recalculate the total and the mean (and std dev if
   * enabled)
   */
  bool write(buff_type value) {
    ...
    _total -= (long double)
        _buff[_writeIndex & _buffSizeMask]; // remove last value from total
   ...
   */
  uint16_t getSize(void) { return _buffSize; }

  /**  Get the current read position within the buffer
   */
  uint16_t getReadIndex(void) { return _readIndex; }

  /**  Get the current write position within the buffer
   */
  uint16_t getWriteIndex(void) { return _writeIndex; }
